...
import { startSession, getSession, setUserNotifier } from './sessionManager.js';
...
const pendingByPhone = new Map(); // phone -> chatId (le temps de l'inscription)
const phoneByChat = new Map();    // chatId -> phone (pour /status sans réécrire le numéro)
const chatIdByPhone = new Map();  // phone -> chatId, persistant, pour te prévenir plus tard (Bad MAC, logout...)
...
export function startTelegramBridge(onMessage) {
  ...
  bot.on('message', async (msg) => {
    ...
    if (existing && existing.status === 'connected') {
      ...
      chatIdByPhone.set(phone, msg.chat.id);
      return;
    }
    ...
    try {
      ...
      await bot.sendPhoto(msg.chat.id, dataUrlToBuffer(entry.qrDataUrl), {
        caption: 'Scanne ce QR dans WhatsApp > Appareils liés > Lier un appareil. Tu as 5 minutes.'
      });
      ...
      const pollInterval = setInterval(async () => {
        ...
        if (current?.status === 'connected') {
          ...
          clearInterval(pollInterval);
          pendingByPhone.delete(phone);
          await bot.sendMessage(msg.chat.id, "❌ La connexion a échoué. Renvoie-moi ton numéro pour réessayer.").catch(() => {});
        }
      }, 4000);
      // Ne surveille pas indéfiniment si la personne abandonne en cours de route —
      // et surtout, on la prévient au lieu de la laisser dans le silence.
      setTimeout(async () => {
        clearInterval(pollInterval);
        if (!settled) {
          pendingByPhone.delete(phone);
          await bot.sendMessage(msg.chat.id, '⌛ Le QR a expiré sans être scanné. Renvoie-moi ton numéro pour en avoir un nouveau.').catch(() => {});
        }
      }, 5 * 60 * 1000);
    } catch (error) {
  ...
  // Prévient directement la personne concernée (Bad MAC, logout, échec de
  // reconnexion) via le chat Telegram où elle s'est inscrite la dernière fois.
  setUserNotifier(async (phone, text) => {
    const chatId = chatIdByPhone.get(phone);
